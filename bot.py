...
def game_embed(g: Game, extra_line: Optional[str] = None) -> discord.Embed:
    ...
    players = f"<@{g.challenger_id}> vs " + (f"<@{g.opponent_id}>" if g.opponent_id else "**(waiting for opponent)**")
    embed.add_field(
        name="Players",
        value=players,
        inline=False,
...
async def deathroll(
    ...
    start: Optional[app_commands.Range[int, 2, 1_000_000]] = None,
    thread: Optional[bool] = True,
):

    start_val = start if start is not None else DEFAULT_START
    ...
    async def accept(i: discord.Interaction):
        # Open challenge: first non-challenger human becomes opponent
        if g.opponent_id is None:
            if i.user.bot:
                return await i.response.send_message("Bots can't accept.", ephemeral=True)
            if i.user.id == g.challenger_id:
                return await i.response.send_message("You can't accept your own open challenge.", ephemeral=True)
            g.opponent_id = i.user.id
    
        # Targeted challenge: only the target can accept
        else:
        ...
        if g.use_thread and isinstance(i.channel, discord.TextChannel):
            try:
                th = await i.channel.create_thread(
                    ...
                    type=discord.ChannelType.public_thread,  # 👈 keep public
                )
                channel_id = th.id
    ...
    async def decline(i: discord.Interaction):
        if g.opponent_id is None:
            if i.user.id != g.challenger_id:
                return await i.response.send_message("Only the challenger can cancel this open challenge.", ephemeral=True)
            msg = "Challenge cancelled."
        else:
            # Targeted: only the challenged user can decline
            if i.user.id != g.opponent_id:
    ...
    if opponent is None:
        content = (
            f"🎲 {interaction.user.mention} started an **open** deathroll challenge! @here\n"
            f"Start: **{start_val}**\n"
            "First person to **Accept** becomes the opponent."
        )
        allowed = discord.AllowedMentions(everyone=True, users=True, roles=False)
    else:
        content = (
            f"🎲 {interaction.user.mention} challenges {opponent.mention} to a deathroll!\n"
            f"Start: **{start_val}**"
        )
